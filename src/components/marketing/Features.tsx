...
const featureIcons = [
  ...
  <svg key="cloud" className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M2.25 15a4.5 4.5 0 004.5 4.5H18a3.75 3.75 0 001.332-7.257 3 3 0 00-3.758-3.848 5.25 5.25 0 00-10.233 2.33A4.502 4.502 0 002.25 15z" /></svg>,
  <svg key="shield" className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z" /></svg>,
];

const featureKeys = [
  { title: "groupTasks", desc: "groupTasksDesc", icon: 0 },
  { title: "reminders", desc: "remindersDesc", icon: 1 },
  { title: "nlp", desc: "nlpDesc", icon: 2 },
  { title: "checklists", desc: "checklistsDesc", icon: 8 },
  { title: "comments", desc: "commentsDesc", icon: 9 },
  { title: "templates", desc: "templatesDesc", icon: 3, pro: true },
  { title: "recurring", desc: "recurringDesc", icon: 4, pro: true },
  { title: "calendar", desc: "calendarDesc", icon: 5 },
  { title: "offline", desc: "offlineDesc", icon: 10 },
  { title: "widget", desc: "widgetDesc", icon: 6 },
  { title: "security", desc: "securityDesc", icon: 11 },
  { title: "multilang", desc: "multilangDesc", icon: 7 },
];
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {featureKeys.map((feature) => (
            <div
              key={feature.title}
              className="group p-6 rounded-2xl bg-surface-1 border border-outline hover:border-primary/40 transition-all"
              style={{ boxShadow: 'var(--shadow-1)', transitionDuration: 'var(--dur-2)', transitionTimingFunction: 'var(--ease)' }}
            >
              <div className="flex items-start justify-between gap-3 mb-4">
                <div className="w-12 h-12 rounded-xl bg-primary-light flex items-center justify-center text-primary group-hover:scale-105 transition-transform" style={{ transitionDuration: 'var(--dur-2)' }}>
                  {featureIcons[feature.icon]}
                </div>
                {feature.pro && (
                  <span className="px-2.5 py-1 rounded-full bg-warmth-soft text-warmth-deep text-xs font-semibold shrink-0">
                    {t("proBadge")}
                  </span>
                )}
              </div>
